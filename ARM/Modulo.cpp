...
void configure_i2c_slave(uint8_t address);
...
bool _ModuloWrite(const ModuloWriteBuffer &buffer) {
...
bool _ModuloRead(uint8_t command, const ModuloWriteBuffer &writeBuffer, ModuloReadBuffer *readBuffer) {
...
bool ModuloHandleDataRequested(uint8_t address, uint8_t *data) {
    ...
    for (int i=0; i < 32; i++) {
        if (!_readBuffer.GetNextByte(data+i)) {
            break;
        }
    }
	return true;
...
bool ModuloHandleDataReceived(uint8_t address, uint8_t *data) {
    ...
    for (int i=0; i < 32; i++) {
        _writeBuffer.Append(data[i]);
        if (_writeBuffer.IsValid()) {
            return _ModuloWrite(_writeBuffer);
        }
    }
	return false;
}
